...
@section('content')
    <div id="kt_content_container" class="container-xxl">
        <form action=" {{route('units.store',['course_id'=>$course->id])}} " method="post" class="form d-flex flex-column flex-lg-row" enctype="multipart/form-data">
            @csrf
                ...
                <div class="card card-flush py-4">
                    ...
                    <div class="card-body pt-0">
                        <div class="mb-10 fv-row">
                            ...
                            <input type="text" name="name" class="form-control mb-2" placeholder=" Nhập tên bài học" value="{{old('name')}}"/>
                            @error('name')
                        ...
                        <div class="row">
                            <div class="mb-10 col-6" style="display:inline-block">
                                ...
                                <input type="file" name="file" class="form-control mb-2" placeholder="Chọn tài liệu cho khoá học..." value="{{old('file')}}" accept=".csv,.doc,.docx,.log,.pdf,.ppt,.pptx,.rtf,.txt,.xls,.xlsx,.xml"/>
                                @error('file')
                                <div style="color: red;" class="">{{$message}}</div>
                                @enderror
                            </div>
                            <div class="mb-10 col-6" style="display:inline-block">
                                ...
                                <input value="{{old('index')}}" class="form-control mb-2" type="number" min="1" name="index" placeholder="thêm số thứ tự">
                                @if(!empty($error))
                       ...
                       @if($course->type===\App\Models\Course::TYPE_ONDEMAND) <div id="video__course" class="col-6 key_live">
                            <div class="card card-flush py-4">
    {{--							<!--begin::Card header-->--}}
                                <div class="card-header">
    {{--								<!--begin::Card title-->--}}
    ...
    {{--									<!--begin::Preview existing avatar-->--}}
                                        <div style="display: inline-block; border: 3px solid #fff; box-shadow: 0 0.1rem 1rem 0.25rem rgb(0 0 0 / 5%);" class="video-prev ">
                                            <video id="video_course" height="500" width="500" class="video-preview" controls="controls"></video>
                                        </div>
